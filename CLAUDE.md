...
- **Task worktrees → `<userData>/worktrees/<taskId>/`**, with the registry naming them at `<userData>/worktrees.json` (`apps/ui/src/main/worktree-service.ts`). They sit beside `daemon.json` and `children.json` without being the daemon's: every git call this app makes belongs to the ELECTRON MAIN process, so main creates the worktree and the branch and the daemon is only told the path afterwards, recording it on `Task.worktreePath` and running the agent's chat with it as `cwd`. A file rather than memory for `children.json`'s reason — a force-quit or a SIGKILL runs no cleanup, and the next launch is the only thing left that can (`reapOrphanedWorktrees`, called before any task can start one) — and an unreadable or malformed registry reads as EMPTY, which is the safe direction, since the only thing it authorizes is a delete. It authorizes nothing outside that one directory: the removal is bounded to `<userData>/worktrees/` by path rather than by what the file claims. The BRANCH is never removed with the worktree — it holds the work, which is the entire point of the run, and that is what makes the collection safe: when a run SETTLES, main commits whatever the agent left uncommitted onto the task's own branch and only then removes the directory, so nothing in there is the last copy of anything. The hooks are RUN rather than skipped (`--no-verify` is refused project-wide), so a repository whose hooks reject a half-finished tree simply keeps its worktree — the same for a machine with no git identity, since one is never invented here. A commit that could not be made is never a reason to remove anyway.
