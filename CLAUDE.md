...
- `v1/agents/` — the shared agent-execution substrate (M2): `agents.module.ts` + `chat.types.ts` at the root, then `adapters/`, `controllers/`, `services/`, `dao/`, `dto/`, `utils/`.
  - `adapters/` — the abstract `AgentAdapter` base + `adapter.types.ts` (the CLI-agnostic contract, incl. `AdapterConfig`/`AdapterQuestion`/`TurnDriver`/`TerminalCommandResult`) + `utils/skill-scan.utils.ts` (the shared on-disk walk, free of any one CLI's paths), then one directory per PROTOCOL several CLIs could speak and one per CLI — see `.claude/rules/agent-adapters.md`. **Every static fact about a CLI is one `AdapterConfig`**, assembled by that adapter's own `getConfig()`, so `listSkills`, `listEfforts`, `listReportedCommands`, `supportsLiveStream`, `resolveApprovalMode` and `terminalCommand` are CONCRETE on the base over config; only `getConfig`, `buildArgs`, `mapMessage` and `listModels` are abstract.
...
- `v1/terminals/` — the terminal mirror: ONE kind, a `claude --model <model> --resume <agentSessionId>` PTY showing the agent's own interactive TUI on this run's conversation. A second, read-only `live` kind used to sit beside it (the headless turn's raw stdout tee'd through `AgentTurnInput.mirror`) and was DELETED with its whole plane — `TurnMirrorService`, the sink on the turn input, the `spawn-cli` tee: it followed the conversation, but what it showed was the CLI's WIRE format (claude's stream-json), which is not a terminal. **Staying in sync is a re-spawn, and it happens DURING a turn, not only at its end**: `TerminalsService` subscribes to the item bus and asks `TerminalSessionsService.refreshTarget` to re-read on EVERY item (`utils/mirror-refresh.ts` classifies the item, marking a chat's `turn_complete` and a workflow node's terminal `status` as `immediate`). Re-spawning is the only mechanism the CLI offers — probe-measured, an already-open TUI grows by 0 bytes while a headless turn runs on the same session, because it reads the transcript once at startup — but the transcript itself grows AS the turn works (probe-measured: a 34s turn appended 11 → 15 → 16 → 19 → 20 → 22 → 25 lines), so re-reading mid-turn genuinely shows new work. Wiring it to the turn's settle instead, as the first cut did, left a mirror opened during a long turn frozen for the whole of it. The re-read is **spawn-first**: the replacement renders OFF-SCREEN while the current child keeps the user's terminal, and only a finished screen is swapped in — one write carrying the clear plus the whole render, so the client paints no blank frame. It cost 3s of empty terminal per refresh when the clear came first (probe-measured: `claude --resume` on a 200KB transcript takes ~780ms to first byte and ~3.0s to settle). The cost is bounded where the cost is known — in the session layer, not at the trigger: at most one re-read per 10s, only while a client is attached, and never within 30s of the user typing into the mirror (a re-spawn is a new process and would take their half-typed line with it) — where "typing" means the RENDERER vouched for a real key or paste event via the `typed` flag on the WS `input` payload, never merely "bytes arrived on the input channel": a terminal emulator answers the TUI's own queries down that same channel (claude asks for Device Attributes on EVERY render and xterm.js replies by itself), and counting those as typing re-armed the grace on every render, so the mirror never refreshed again. A replacement that fails to spawn, dies early, or draws nothing leaves the working mirror alone: a stale mirror beats a dead one. Layout: `terminals.module.ts` + `terminals.types.ts` at the root, then `services/` (`terminal-sessions.service` — session lifecycle, scrollback replay, byte fan-out, settling, and the spawn-first refresh swap + its throttle; a session OUTLIVES its child, so ONE `ProcessRegistry` claim under `terminal:<id>` covers it whichever child is current, and a kill outranks a pending refresh; SIGHUP→group-SIGKILL escalation, exited-session TTL; `terminals.service` — resolves a run/node to that spawn via the stored `node_state` session id (the model rides along so the mirror opens on the SAME model the chat runs, not the CLI default; `AgentAdapter.terminalCommand` takes a `TerminalCommandInput`), and owns WHEN a mirror is stale), `gateways/terminals.gateway.ts` (the `/terminals` Socket.IO namespace on the SAME `/ws` engine.io instance — attach/replay, input, resize, detach), `controllers/`, `dto/`, `utils/`. The invocation comes from `AgentAdapter.terminalCommand` (config-driven, per CLI); cursor-agent's `terminal: null` in `cursor-acp.adapter.ts`'s config literal says that CLI has no such mirror, and `TerminalsService` maps the adapter's refusal onto `TERMINAL_UNSUPPORTED` / `TERMINAL_SESSION_UNAVAILABLE` so the adapter layer stays free of HTTP exceptions — so **a cursor-agent run has no terminal at all**, and the renderer hides the button rather than offering one that fails on click. Sessions are ephemeral (in-memory only — a mirror is not history).
...
| `/v1/terminals*` | terminal mirror sessions: `POST /` (open for a run/node — the agent's own CLI resumed on that conversation; refuses `TERMINAL_UNSUPPORTED` for a CLI with no such invocation), `GET /`, `GET /:id`, `DELETE /:id` (kill) | bearer token (`LoopbackTokenGuard`) |
