...
- `v1/terminals/` — the M4 live PTY terminal mirror: `terminals.module.ts` + `terminals.types.ts` at the root, then `services/` (`pty.service` — node-pty sessions registered with the shared `ProcessRegistry` under `terminal:<id>`, scrollback replay buffer, SIGHUP→group-SIGKILL escalation, exited-session TTL; `terminals.service` — resolves a run/node to a `claude --model <model> --resume <agentSessionId>` spawn via the stored `node_state` session id (the model rides along so the mirror opens on the SAME model the chat runs, not the CLI default; `AgentAdapter.terminalCommand` takes a `TerminalCommandInput`)), `gateways/terminals.gateway.ts` (the `/terminals` Socket.IO namespace on the SAME `/ws` engine.io instance — attach/replay, input, resize, detach), `controllers/`, `dto/`. The mirror invocation itself comes from `AgentAdapter.terminalCommand` (config-driven, per CLI); cursor-agent's `terminal: null` in `cursor-acp.adapter.ts`'s config literal IS the deferred-scope statement, and `TerminalsService` maps the adapter's refusal onto `TERMINAL_UNSUPPORTED` / `TERMINAL_SESSION_UNAVAILABLE` so the adapter layer stays free of HTTP exceptions. Sessions are ephemeral (in-memory only — a live mirror is not history).
