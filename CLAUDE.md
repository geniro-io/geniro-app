...
| `DELETE /v1/chats/:runId` | **destructive and irreversible**: cancels a running turn, then hard-deletes the run's `runs`/`items`/`node_state` rows with the `softDelete` filter disabled, removes `<userData>/attachments/<runId>/`, kills its live PTY mirrors and revokes its call tokens. Chat runs only — a workflow run is rejected, since deleting one through this route would skip the graph executor's teardown | bearer token (`LoopbackTokenGuard`) |
