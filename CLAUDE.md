...
- `v1/agents/` — the shared agent-execution substrate (M2): `agents.module.ts` + `chat.types.ts` at the root, then `adapters/`, `controllers/`, `services/`, `dao/`, `dto/`, `utils/`.
  - `adapters/` — the abstract `AgentAdapter` base + `adapter.types.ts` (the CLI-agnostic contract, incl. `AdapterConfig`/`AdapterQuestion`/`TurnDriver`/`TerminalCommandResult`) + `utils/skill-scan.utils.ts` (the shared on-disk walk, free of any one CLI's paths), then one directory per PROTOCOL several CLIs could speak and one per CLI — see `.claude/rules/agent-adapters.md`. **Every static fact about a CLI is one `AdapterConfig`**, assembled by that adapter's own `getConfig()`, so `listSkills`, `listEfforts`, `listReportedCommands`, `supportsLiveStream`, `resolveApprovalMode` and `terminalCommand` are CONCRETE on the base over config; only `getConfig`, `buildArgs`, `mapMessage` and `listModels` are abstract.
    - `acp/` — the agent-agnostic **Agent Client Protocol** client: `acp.types.ts` (wire shapes), `acp-jsonrpc.ts` (framing), `acp-driver.ts` (the per-turn state machine). It knows nothing about cursor; a second ACP-capable CLI composes it rather than copying the protocol.
...
- **Pasted images → files** under `<userData>/attachments/<runId>/` (`AttachmentStoreService`); only the `{id, mediaType}` row rides the message item's payload, keeping blobs out of the DB. Delivery is per CLI: claude gets real base64 image **content blocks** in its stream-json stdin (probe-verified on 2.1.220 — no `Read` tool, no permission gate; `claude-images.utils.ts`). **The ACP path delivers nothing today** — `input.images` is read only by the claude adapter, so an image pasted into a cursor chat is stored and shown in the transcript but never reaches the CLI. The legacy `-p` transport told cursor the file PATHS; that was deleted with it and not re-hung on ACP's `session/prompt` content blocks. Fix it in `acp/acp-driver.ts` (the prompt it sends), not at a call site.
