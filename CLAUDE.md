...
| `GET /v1/agents/mcp` | the MCP servers one agent loads in a given cwd, with the live health the CLI reported (`AgentMcpService` → `AgentAdapter.listMcpServers`); `refresh=true` re-dials instead of serving the cached reading. A CLI that cannot be listed answers `unavailableReason` rather than an empty list, so "no servers here" and "this CLI cannot say" stay distinguishable without any caller branching on agent kind | bearer token (`LoopbackTokenGuard`) |
