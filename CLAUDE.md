...
- `v1/agents/` — the shared agent-execution substrate (M2): `agents.module.ts` + `chat.types.ts` at the root, then `adapters/`, `controllers/`, `services/`, `dao/`, `dto/`, `utils/`.
  ...
  - Exported to the graphs module: event bus, approval registry, partial-stream, `ClaudeProbeService`, process registry, `RunTeardownService`, `SkillHarvestStore`, `McpSettingsStore`, `CursorMcpCleanupService`, the three DAOs, both adapters, and the adapter registry.
...
- `v1/terminals/` — the terminal mirror, in TWO kinds (`TerminalKind`, picked per session). **`live`** (the default) mirrors the run's OWN headless turns: it attaches to the `TurnMirrorService` buffer above and is READ-ONLY, since there is no second process to type at — it follows the conversation across turns and works for EVERY agent kind, because raw bytes need no `--resume` support. **`interactive`** is the original separate `claude --resume` PTY: you can type at it, but being a different process it replays the transcript at startup and then cannot advance while the chat's own turn is running — the standstill `live` exists to close. Layout: `terminals.module.ts` + `terminals.types.ts` at the root, then `services/` (`terminal-sessions.service` — owns BOTH kinds, since session lifecycle, scrollback replay, byte fan-out and settling are not PTY-specific; only spawn/write/resize/kill are, and each is guarded on `session.pty` being non-null (a `live` session has none, and a pid-shaped default reaching the group-SIGKILL would signal the daemon's own group). PTY sessions register with the shared `ProcessRegistry` under `terminal:<id>`, a live one claims nothing (it spawns nothing); SIGHUP→group-SIGKILL escalation, exited-session TTL for both; `terminals.service` — resolves a run/node to a `claude --model <model> --resume <agentSessionId>` spawn via the stored `node_state` session id (the model rides along so the mirror opens on the SAME model the chat runs, not the CLI default; `AgentAdapter.terminalCommand` takes a `TerminalCommandInput`) for the `interactive` kind, and resolves a `live` one straight off the mirror buffer — needing no agent kind, no resumable session, no model and no cwd), `gateways/terminals.gateway.ts` (the `/terminals` Socket.IO namespace on the SAME `/ws` engine.io instance — attach/replay, input, resize, detach), `controllers/`, `dto/`. The INTERACTIVE invocation comes from `AgentAdapter.terminalCommand` (config-driven, per CLI); cursor-agent's `terminal: null` in `cursor-acp.adapter.ts`'s config literal says that CLI has no such mirror, and `TerminalsService` maps the adapter's refusal onto `TERMINAL_UNSUPPORTED` / `TERMINAL_SESSION_UNAVAILABLE` so the adapter layer stays free of HTTP exceptions. The LIVE kind asks no adapter anything — it is the bytes off whichever CLI ran, which is why cursor-agent now has a terminal at all. Sessions are ephemeral (in-memory only — a live mirror is not history).
...
| `GET /v1/agents/mcp` | the MCP servers one agent loads in a given cwd, with the live health the CLI reported (`AgentMcpService` → `AgentAdapter.listMcpServers`); `refresh=true` re-dials instead of serving the cached reading. only `servers: []` with a NULL `unavailableReason` asserts that the folder has none — a non-null reason covers both a CLI that cannot be listed at all and a read that failed or could not be parsed, so "no servers here" and "we are not saying" stay distinguishable without any caller branching on agent kind. Each row also carries `scope` / `disabled` / `toggleUnavailableReason`, composed from `AgentAdapter.readMcpFolderFacts` + `McpSettingsStore` on EVERY exit path (cache hit included) | bearer token (`LoopbackTokenGuard`) |
| `PUT /v1/agents/mcp` | switch one server on or off for one `(agent, cwd)`, persisted in geniro's own `<userData>/mcp-settings.json` and answered with the recomposed listing. REFUSES rather than silently no-opping: a non-project-scope server has no verified disable mechanism, and one the user disabled in their OWN claude settings can never be re-enabled from here (the CLI UNIONs the two `disabledMcpjsonServers` lists — probe-verified on 2.1.220) | bearer token (`LoopbackTokenGuard`) |
...
| `/v1/terminals*` | terminal mirror sessions: `POST /` (open for a run/node; `kind` selects `live` — the default, the run's own turns tee'd raw — or `interactive`, the `--resume` PTY), `GET /`, `GET /:id`, `DELETE /:id` (kill) | bearer token (`LoopbackTokenGuard`) |
...
- **MCP toggles → `<userData>/mcp-settings.json`** — geniro's own file and the ONLY one this feature writes. The user's `~/.claude.json`, `.claude/settings*.json` and project `.mcp.json` are READ at most, never written. Atomic (tmp+rename via `utils/atomic-file.ts`); a malformed file degrades to "nothing disabled" rather than failing a turn.
