...
| `POST /v1/mcp/:runId/:nodeId` | the agent-to-agent call runtime: a stateless MCP JSON-RPC endpoint (`initialize` / `tools/list` / `tools/call` for `call_agent` + `await_agent`; a live probe run id is served a single `echo` tool instead) scoped to one run's one caller node | that caller node's **per-node call token** OR the launch token (`LoopbackTokenGuard`) |
