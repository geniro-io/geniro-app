...
| `GET /v1/agents/skills` | skills + slash commands one agent kind accepts in a given cwd (on-disk scan, project + `~`) — feeds the composer's `/` autocomplete | bearer token (`LoopbackTokenGuard`) |
