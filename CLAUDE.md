...
**Agents (M2+)** are driven **headlessly via their CLIs only** — `claude -p`, and `cursor-agent` over either its one-shot `-p --output-format stream-json` stream or its first-party **ACP** server (`cursor-agent acp`, opt-in via `GENIRO_CURSOR_ACP=1`). No SDKs, no LangGraph host-side, no Python.
...
**The cursor transport (ACP)** — `cursor-agent` can be driven two ways, chosen ONCE by DI (`CURSOR_ADAPTER` in `agents.module.ts`) from `environment.cursorAcp` (`GENIRO_CURSOR_ACP=1`, default off until verified against installed cursor-agent builds); every consumer injects the token typed as `AgentAdapter`, never a concrete class. The legacy `CursorAdapter` runs one-shot `-p --output-format stream-json`. `CursorAcpAdapter` runs `cursor-agent acp` — Cursor's first-party Agent Client Protocol server — over JSON-RPC on stdio via the agent-agnostic `adapters/acp/` client. What changes under ACP: **real permission prompts** (`session/request_permission` is an ACP baseline, so `ask`/`acceptEdits` stop degrading to `--force`), **client-supplied MCP servers** (a caller node's endpoint travels in `session/new`, so no `.cursor/mcp.json` merge, no MCP-trust probe, and the call token rides an HTTP header inside a stdin frame rather than a file on disk), and a **typed event stream** (`session/update` instead of version-volatile NDJSON). What does NOT change: one turn is still one process (spawn → handshake → prompt → stop reason → exit), so `ProcessRegistry`, cancel, and the executor's fan-out are untouched. Known ACP gaps, all surfaced as `notice` events rather than dropped: per-session model selection, `--trust`, and Cursor's blocking `cursor/*` extension requests (declined in-protocol with `-32601`). The legacy path and its `.cursor/mcp.json` machinery stay until the ACP path is verified on a real binary.
