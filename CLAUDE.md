...
**ASK IN SYMBOL NAMES, NOT PROSE.** The tool's own description says a natural-language question works; its author's `AGENTS.md` says the opposite — *"accepts a precise bag of symbol names… not natural-language descriptions. Fuzzy or descriptive queries return irrelevant results"* — and the author is right. Query tokens are matched against symbol NAMES, so a sentence is a bag of the commonest names in any codebase: `error`, `failed`, `status`, `session`. MEASURED here on one index — asking *"…stream-json result event is_error subtype — how an error item is emitted…"* ranked `base-logger.ts` and `sink-logger.ts` first (both carry an `error()` method) and never reached `claude-turn.driver.ts`, while `ClaudeTurnDriver mapClaudeMessage errorDetail` put `claude-message.utils.ts`, `error-detail.ts` and `claude-turn.driver.ts` in the top three. Same index, same ~200 candidate symbols; only the ORDER differed — and the output budget cuts after ~3 files, so the order IS the answer. For a flow, name the symbols spanning it (`mutateElement renderScene`). A PascalCase token biases an overloaded name toward that type's own definition. `maxFiles` (default 12) widens how much source is shown; it does not rescue a query that ranked the wrong files.

**`codegraph.json` at the repo root carries `deprioritize`** — paths that stay indexed and findable but stop outranking product code: the stories, `__tests__/`, the generated client and the vendored `packages/`. Verified in both directions after adding it — the prose query above stopped returning the loggers, and `BaseDao hardDeleteIncludingSoftDeleted` still answers with `packages/mikroorm/src/base.dao.ts` FIRST. It changes rank, never visibility; reach for `exclude` when something should be gone. Needs codegraph ≥ 1.6.0.
...
The public allowlist is **`/health` and `/v1/artifacts`** (`PUBLIC_PREFIXES` in `auth/token.guard.ts`), matched at segment boundaries so a sibling route like `/health-debug` doesn't inherit it. The artifact page is not ungated — it is gated somewhere the guard cannot reach: it loads as an `<iframe src>`, and no API lets a frame's request carry a bearer header, so the credential rides the URL as a per-ARTIFACT key the store minted (256 bits) rather than the launch token, which is the master key of the whole daemon and does not belong in a URL handed to a page an agent wrote. `ArtifactStoreService.read` takes that key as a required argument and compares it in constant time, so there is no path that reads a document without presenting one; it is listed in the guard rather than verified there because the keys live on disk beside the pages and outlive a restart, which an in-memory registry could not answer for. `/metrics` and `/swagger-api` are **token-gated**: with a deterministic default port, any web page could otherwise read the daemon's Prometheus internals and full API schema cross-origin. Every non-allowlisted route requires the `Bearer <token>` header — the launch token, or, **only** on `/v1/mcp/<runId>/<nodeId>`, that node's own per-node call token (minted when the node first needs the endpoint — for a chat that is its first turn, since every chat is now handed the endpoint for the render tools (`report_findings`, `show_chart`, `show_metrics`, `show_comparison`, `show_gallery`, `show_artifact`, `propose_patch`, `propose_plan`) — revoked at run teardown, keyed by `(runId, nodeId)` so one callee child can't open another node's route). The WS channel is a NestJS Socket.IO gateway (`@WebSocketGateway({ path: '/ws' })`) installed via the `IoAdapter` in `main.ts`; the renderer (`socket.io-client`) sends the per-launch token in the Socket.IO handshake `auth` payload (browsers can't set headers on a WS handshake). The HTTP `LoopbackTokenGuard` doesn't see Socket.IO traffic (engine.io intercepts `/ws` before Nest routing), so each gateway owns its own WS auth — via the one shared `auth/ws-auth.ts` → `enforceWsHandshakeAuth` (constant-time `safeEqual`, disconnects on mismatch), called from `handleConnection` in the notifications gateway. It stays extracted precisely so a second gateway cannot reintroduce a divergent copy of the check.
...
**The user's half is one global blob, snapshotted per run.** It lives in `settings.json` (`Settings.customInstructions`), rides chat create and workflow-run start on the wire under one exported `CustomInstructionsSchema` (no length limit, but refusing C0 control characters — a NUL reaches `spawn` as argv, where node throws SYNCHRONOUSLY, and the snapshot would repeat that throw on every turn of that chat forever), and is stored on `Run.customInstructions`. Snapshot rather than a live read, and the difference is visible: editing the box changes the NEXT chat. That is deliberate — `AgentAdapter.sessionKey` hashes the value, so a live read would invalidate the kept CLI process mid-conversation and respawn it, taking the user's MCP servers down between two messages. A graph node's `role` outranks it (specific beats general), so the two are PEER fields rather than one joined string. The snapshot's escape hatch is `POST /v1/chats/forget-custom-instructions` behind an explicit Settings button — clearing the box changes only the next run, so without it text a user retracts keeps being sent by every chat opened beforehand; it is a press rather than a side effect of the edit precisely because it discards that per-run guarantee.

**On ACP the WHOLE block is sent once per SESSION, never once per turn.** Prompt text is part of the conversation there, so a block one turn carries stays in the window for every turn after it. It used to be only the preamble that was withheld after the first turn, while the user's instructions, the task instructions, the wired blocks, the role and the call surface rode EVERY message — a Dev Team node with a ~6KB role and a ~15KB settings block added ~5k tokens of duplicate instructions to its own window per turn. So `AcpTurnDriver.composePrompt` sends the block only when its TEXT differs from `AcpSession.deliveredInstructions`, which the two ways a session can already hold a block keep current: a prompt WRITTEN on this kept process records what it carried, and a successful `session/load` reads the newest `<host-context>` block back out of the replay (`readHostContext` — cursor-agent replays each prior user message whole as one `user_message_chunk`, read in its own `replayAgentTurn`). Compared as text rather than flagged, so a block that CHANGED — a call surface this process was not granted, forgotten custom instructions, re-resolved label instructions — is still delivered and supersedes the older one; a replay that cannot be parsed, or a refused load that fell back to `session/new`, re-sends. A `/compact` needs nothing: this CLI's compaction replaces the session, and a fresh session holds nothing. Claude needs none of this — its block rides `--append-system-prompt`, outside the conversation, and a turn's stdin carries only the user's words.
...
- **Published artifacts → files** under `<userData>/artifacts/<runId>/<artifactId>/v<N>.html` (`ArtifactStoreService`), beside a `meta.json` holding that artifact's version and its per-artifact serving key. The `show_artifact` row carries only the descriptor — id, version, title, summary and key — so a 512KB page is never replayed with the transcript. EVERY version is kept: a transcript row names the version it announced, so a republish cannot change what an older card in the scrollback opens. Dropped with the run by `RunTeardownService`, exactly as attachments are.
- **Task worktrees → `<userData>/worktrees/<taskId>/`**, with the registry naming them at `<userData>/worktrees.json` (`apps/ui/src/main/worktree-service.ts`). They sit beside `daemon.json` and `children.json` without being the daemon's: every git call this app makes belongs to the ELECTRON MAIN process, so main creates the worktree and the branch and the daemon is only told the path afterwards, recording it on `Task.worktreePath` and running the agent's chat with it as `cwd`. A file rather than memory for `children.json`'s reason — a force-quit or a SIGKILL runs no cleanup, and a later pass is the only thing left that can (`reapFinishedWorktrees`, run each time a daemon comes up) — and an unreadable or malformed registry reads as EMPTY, which is the safe direction, since the only thing it authorizes is a delete. It authorizes nothing outside that one directory: the removal is bounded to `<userData>/worktrees/` by path rather than by what the file claims. **A worktree lives until its card's work is FINISHED** — the card is Done and no run is working in it (`isWorkFinished` in `v1/tasks/utils/work-finished.ts`, announced as the `work-finished` reason on `task_changed` and answered in bulk by `POST /v1/tasks/finished`) — and never merely until its run SETTLES, which is when it was collected first. A task's run is an ordinary chat: the user continues it after review, Retry re-sends into it, and the CLI opens a turn of its own when a background command it started reports back, all in that directory. REPORTED as a thread whose CLI continuation failed in 0ms and whose every later message was refused `cwd does not exist`, with the screenshots its galleries showed gone too — gitignored, so in no commit. A second press of Run CONTINUES in the task's standing worktree (`prepareWorktree` → `reused`) rather than refusing its uncommitted work, and a start that is then refused leaves a reused one alone. The BRANCH is never removed with the worktree — it holds the work, which is the entire point of the run: at collection main commits whatever the agent left uncommitted onto the task's own branch and only then removes the directory, so every TRACKED change survives it; what `.gitignore` excludes does not, which is exactly why collection waits for the user to call the card Done. The hooks are RUN rather than skipped (`--no-verify` is refused project-wide), so a repository whose hooks reject a half-finished tree simply keeps its worktree — the same for a machine with no git identity, since one is never invented here. A commit that could not be made is never a reason to remove anyway.
...
- **No cloud SERVICE of geniro's own, and no remote or multi-machine code paths.** Everything geniro runs, stores and computes is on the user's machine: there is no geniro backend, no telemetry, no account, and nothing syncs between machines. Two shapes of outbound call are allowed, and they are genuinely different — one predicate covering both would either condemn a shipped feature or widen into permitting any HTTP GET at all:
  ...
  1. **The user's own credential, the user's own data.** `main/github-prs.ts` runs the user's `gh` CLI against their own repositories. geniro holds no token — it borrows a login the user already made, and degrades to silence when that login or the network is absent. `v1/agents/services/cursor-usage.service.ts` is the SECOND case and goes one step further, so the difference is worth stating: it reads the user's own `cursor-access-token` out of the Keychain and makes the request ITSELF, where the `gh` path shells out to a CLI that holds its own login. It was admitted because the figure exists nowhere else — measured 2026-08-31, cursor-agent sends no `usage` on the ACP wire, emits no `usage_update` (the name appears in its bundle only inside the protocol schema, with no emitter), records no charge under `~/.cursor`, and the amount is not derivable from tokens either, carrying an enterprise discount and a separate token fee. Its shape is what keeps it inside the rule: ONE fixed host, the user's own account, a poll that covers every conversation at once rather than a request per turn, the token used for that poll and never stored, and every failure — no Keychain item, a denied prompt, a signed-out account, no network — ending as the "no cost reported" dash the header already draws. Its cadence has TWO floors and the shorter one is the live case: ten minutes for the ambient trigger (a thread was opened, so its figure may as well be current) and sixty seconds while a cursor run is actually producing items, because Cursor bills per REQUEST and the figure moves mid-turn. So the account-wide ceiling is 60 POLLS an hour while an agent is working and 6 otherwise, each poll walking at most `CURSOR_USAGE_MAX_PAGES` pages — the unit is the poll rather than the HTTP request, and the two differ by that page fan-out. It is bounded by the work rather than by a timer, since an idle machine polls nothing at all and a claude-only user polls nothing ever.
  ...
  What stays refused is the thing the rule is actually for: geniro holding a credential of its own, syncing the user's data anywhere, or talking to a machine on someone else's say-so.

  **The LAN gateway is the one INBOUND exception, and it is the user's own machine serving the user's own devices** (`apps/ui/src/main/remote/`, `Settings.remoteAccessEnabled`, on by default). It answers on the local network so a phone on the same Wi-Fi can open this app's own interface and the same live sessions. "No cloud SERVICE" is untouched: nothing leaves the network, there is still no geniro backend and no account, and the second machine is one the user is holding. The clause it genuinely amends is "running a server" — geniro now runs one, on the LAN, and the rest of this bullet is what bounds it. Four properties do that, and each answers a way the exception could widen into the rule it is carved out of:
  - **The DAEMON still binds `127.0.0.1` only** — the bullet below is unweakened and the gateway does not relax it. Every LAN-facing byte goes through this one directory, which reverse-proxies to loopback; so "where is this app reachable from" has exactly one answer to audit.
  - **Nothing is proxied to the daemon for a device that has not paired.** `RemoteGateway.isPaired` gates `/v1/*`, `/health` AND the `/ws` upgrade, not merely the gateway's own routes — the proxy INJECTS the daemon's credential, so an ungated arm there is strictly worse than no gate at all: it presents the daemon's own authorization on a stranger's behalf. The upgrade additionally checks `Origin`, because a WebSocket handshake is exempt from the same-origin policy and the browser attaches the cookie regardless of which page asked. Both were missing in the first cut of this feature and are pinned by tests that go red when the check is removed.
  - **The daemon's token never leaves the machine.** The gateway injects `Authorization` itself and strips whatever the browser sent, and rewrites the token in a `/ws` upgrade URL (which is why `enforceWsHandshakeAuth` now also reads the handshake query). The two channels that could have handed it over instead are closed at the registry: `getDaemonHandle` is `denyRemotely` (it is nothing BUT the credential) and `getStatus` carries an `allowRemotelyRedacted` policy that blanks the token in the remote answer alone. A browser synthesises its own handle from `location` with an empty token — which is also what makes the `/ws` query rewrite the thing that authenticates it.
  - **A new device types a 6-digit code shown on this screen**, bounded per address AND globally — the global ceiling rotates the code when it trips, because a per-address lockout is not a bound at all when picking a new source address is free.
  - **A `Host` header geniro would never hand out is refused** (loopback, private IPv4, link-local/unique-local IPv6, `*.local`). That is what stops a page on the open internet pointing its own domain at the LAN address and driving the gateway from the victim's browser — and it is also what keeps a globally-routable IPv6 address, which macOS hands out via SLAAC, from turning a LAN feature into a public one. The listener binds IPv4 only for the same reason.

  **A PUBLIC address is the user's own press, and it is a TUNNEL CLIENT rather than anything geniro hosts** (`apps/ui/src/main/remote/tunnel.ts`, Settings → Remote access → *Get an address*). It starts a client the user already has installed — `cloudflared` first, `ngrok` as the fallback — pointed at the loopback side of the gateway above, and shows the address that client prints. This paragraph AMENDS the sentence that used to end this bullet ("does NOT license: a relay, a tunnel, …"), which was written before the feature and is now wrong about the tunnel alone; everything else it refused it still refuses. Four things keep the amendment narrow, and the first is what makes the rest true:
  - **geniro hosts NOTHING and holds no credential.** There is still no geniro backend, no account and no rendezvous host: the tunnel is a third-party client, chosen by the user, authenticated (where it authenticates at all) by the user's own login, running as a child of this process on this machine. What geniro contributes is a `spawn` and a regex over the client's own stdout.
  - **Nothing below it is relaxed.** The daemon still binds `127.0.0.1`, the gateway is still the only LAN-facing listener, and a request arriving down the tunnel meets the SAME Host guard, the same pairing gate, the same `Origin` check and the same injected credential as one from the Wi-Fi. Measured through a real tunnel: cloudflared forwards the public hostname verbatim in `Host` and the browser's `Origin` arrives as the public https origin, so both guards see the public name and both had to be widened for it deliberately.
  - **The widening is a `*.suffix` mask that exists only while the tunnel does** (`extraAllowedHosts`, read fresh per request; `wildcardAdmits` in `host-guard.ts`). A mask is forced rather than chosen — every client mints a fresh subdomain per start and ngrok was measured reassigning one mid-session — and it is bounded by refusing a bare `*` and a one-label suffix, since a whole TLD is a zone a stranger can register inside and rebind through. Closing the address narrows the guard in the same instant.
  - **It is opened by a PRESS at the Mac, never by geniro and never by a phone.** `startRemoteTunnel`/`stopRemoteTunnel` are `denyRemotely`, so a paired device — including a stolen one — cannot publish its owner's agents; a launch always starts with no tunnel; and switching remote access off closes it.

  What this exception still does NOT license: a relay or rendezvous host of geniro's own, geniro holding a credential for one, a tunnel geniro opens unasked, or reaching the daemon from anywhere but loopback.
...
- **The daemon binds loopback (`127.0.0.1`) only** and gates every non-public route with the per-launch bearer token. Unchanged by the LAN gateway above, which is a SECOND listener in the Electron main process: reaching the daemon from the network still means going through it, under its Host guard and its pairing gate.
