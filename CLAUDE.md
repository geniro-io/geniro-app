...
| `GET /v1/agents/mcp` | the MCP servers one agent loads in a given cwd, with the live health the CLI reported (`AgentMcpService` → `AgentAdapter.listMcpServers`); `refresh=true` re-dials instead of serving the cached reading. only `servers: []` with a NULL `unavailableReason` asserts that the folder has none — a non-null reason covers both a CLI that cannot be listed at all and a read that failed or could not be parsed, so "no servers here" and "we are not saying" stay distinguishable without any caller branching on agent kind. Each row also carries `scope` / `disabled` / `toggleUnavailableReason`, composed from `AgentAdapter.readMcpFolderFacts` on EVERY exit path (cache hit included), because the LISTING cannot see the toggle — `claude mcp list` reports a disabled server as though it were live | bearer token (`LoopbackTokenGuard`) |
