...
import { CURSOR_MCP_CONFIG_SEGMENTS } from '../cursor.const';
import type {
...
function configPathOf(cwd: string): string {
  return join(cwd, ...CURSOR_MCP_CONFIG_SEGMENTS);
}
...
export function mergeGeniroEntry(
  ...
  const cursorDir = dirname(configPathOf(cwd));
...
export function restoreGeniroEntry(
  ...
  const backup = backupPathOf(cwd);
  try {
    const cursorDirStat = lstatIfExists(dirname(configPathOf(cwd)));
    if (
