 ...
 * `GENIRO_`-prefixed key plus a natively inherited Cursor key. Those carry the
 * daemon's own config and secrets — most
...
export function buildChildEnv(
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (!key.startsWith('GENIRO_') && key !== 'CURSOR_API_KEY') {
      env[key] = value;
    }
  }
  return { ...env, ...extra };
