...
function controller(callBroker = broker()): McpController {
  return new McpController(callBroker, {
    token: 'launch',
    version: '9.9.9',
    startedAt: 0,
    port: 4870,
...
/**
 * Drive the controller over a REAL loopback http server: the SDK's Node
...
async function post(
  ...
  const server = createServer((req, res) => {
    ...
    req.on('end', () => {
      ...
      void target.handle(runId, nodeId, fastifyReq, fastifyReply);
    });
  });
...
describe('McpController', () => {
  ...
  it('refuses GET and DELETE with 405 (each its own registered handler)', () => {
    for (const method of ['getNotAllowed', 'deleteNotAllowed'] as const) {
      const send = vi.fn();
      const header = vi.fn(() => ({ send }));
      const status = vi.fn(() => ({ header }));
      controller()[method]({ status } as unknown as FastifyReply);
