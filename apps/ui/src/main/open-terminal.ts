 ...
 * to LaunchServices.
 *
 * The darwin check throws before anything is written, so a refused call leaves
 * no script behind: the whole mechanism (`.command`, LaunchServices, `open`)
 * is macOS-specific.
 */
async function runInTerminal(cwd: string, body: string): Promise<void> {
