...
import { THREAD_PULL_REQUESTS_SETTLED_FLAG } from './panel-flags';
import { ThreadUiMemoryContext, writeThreadFlag } from './thread-ui-memory';
...
describe('AgentsPanel thread pull requests', () => {
  ...
  it('opens on the fold that the shelf button asked for', async () => {
    ...
    writeThreadFlag('run-1', THREAD_PULL_REQUESTS_SETTLED_FLAG, true);
    const el = render(
      <ThreadUiMemoryContext.Provider value="run-1">
        <AgentsPanel
          terminalReasons={TERMINALS}
          agents={[]}
          threadPullRequests={[threadRow(84, 'privacy copy', 'merged')]}
          onOpenThread={vi.fn()}
        />
      </ThreadUiMemoryContext.Provider>,
    );
