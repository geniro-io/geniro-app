...
function QuestionCard({
   ...
   * reaches the model as `updatedInput.response`, a plain string field of the
   * CLI's own tool input, so there is nowhere in it for bytes to go. The
  ...
  /**
   * The answer as sent, plus a line stating that an image follows.
   *
   * The note is a factual description of the DELIVERY, not words put in the
   * user's mouth: the bytes go out as their own message, and without this the
   * model reads the answer, acts on it, and only meets the screenshot
   * afterwards with no idea it was meant as part of the reply.
   */
    ...
    <Card className="flex flex-col gap-2.5 border-primary/40 p-3">
      ...
      {pending ? (
        <div
          ...
          />
          {attachments.error ? (
            <p className="m-0 text-xs text-destructive">{attachments.error}</p>
          ) : null}
          <Input
