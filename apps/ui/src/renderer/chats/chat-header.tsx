...
/**
 * The control that opens "what changed since this chat started" — a CHIP once
 * there are figures to put on it, the bare icon until then.
 *
 * Stating the size on the control is the whole point: the view answers "what has
 * the agent done to my folder", and that is a question a reader has to open a
 * dialog to ask if the control only carries a glyph. A chip answers the common
 * case — a number and two figures — without opening anything.
 *
 * Two states draw the ICON rather than the chip, and they are different from
 * each other. `null` is "this folder has not been read yet", where a chip would
 * have to invent figures. `files === 0` is a real reading of a folder nothing
 * has touched, where a `0 files +0 −0` chip would spend width to say nothing —
 * the absence of figures IS the zero, and the tooltip says so in words. The
 * control stays in both, because "has anything changed" is worth being able to
 * ask even when the answer turns out to be no.
 */
function ChangesControl({
  ...
  const label =
    summary === null
      ? 'Changed since this chat started'
      : summary.files === 0
        ? 'Nothing has changed in this folder since this chat started'
        : `${summary.files} ${summary.files === 1 ? 'file' : 'files'} changed since this chat started`;
  ...
  return (
    <Chip
      asChild
      interactive
      // ACTIVE rather than muted: `tone` is about whose the value is, and this
      // one is the reader's own folder as an agent has left it — the thing they
      // opened the thread to check. It also sits beside the muted identity
      // figures, where receding would lose it entirely.
      tone="active"
      className="shrink-0">
      <button type="button" aria-label={label} title={label} onClick={onOpen}>
        <GitCompare aria-hidden="true" />
        {/* The file count is the headline — "how much of my tree did it touch"
            — and it never truncates, being three characters at most. */}
        <span className="tabular-nums">{summary.files}</span>
...
export function ChatHeader({
  ...
  onSearch?: () => void;
  /**
   * Open the "what changed since this chat started" view. Absent — a run with no
   * folder, or one this app never stamped a commit for — draws no control, on
   * {@link onSearch}'s rule: there is nothing to compare against, which is not
   * the same as something being wrong.
   */
  onShowChanges?: () => void;
  /**
   * What that view would say, stated on the control itself.
   *
   * Null until the folder's first read lands, which is NOT the same as zero: a
   * chip reading `0 files` before anything was measured claims a reading nobody
   * took. Both of those draw the bare icon; only a real, non-empty answer earns
   * the figures.
   */
