...
describe('hasRunningBackgroundShell', () => {
  // The fixtures stamp every item 2026-08-24T10:00, so a daemon that started
  // before that owns them and one that started after cannot.
  const OWNS_THEM = '2026-08-24T09:00:00.000Z';
  const STARTED_LATER = '2026-08-24T11:00:00.000Z';

  it('sees a detached command that is still going', () => {
    const items = [
      bash('c1', 'pnpm dev', { run_in_background: true }),
      reply('c1', LAUNCHED('bash_1')),
    ];

    expect(hasRunningBackgroundShell(items, OWNS_THEM)).toBe(true);
  });

  it('ignores a FOREGROUND call that never came back', () => {
    // Unanswered because the turn was cancelled or abandoned, not because
    // anything is running — and this reading has to survive a SETTLED run,
    // where that call would otherwise stand for the life of the transcript.
    expect(
      hasRunningBackgroundShell([bash('c1', 'sleep 400')], OWNS_THEM),
    ).toBe(false);
  });

  it('ignores a shell an EARLIER daemon started', () => {
    // The process that owned it is gone, so it cannot still be running.
    const items = [
      bash('c1', 'pnpm dev', { run_in_background: true }),
      reply('c1', LAUNCHED('bash_1')),
    ];

    expect(hasRunningBackgroundShell(items, STARTED_LATER)).toBe(false);
  });

  it('drops it once the shell has reported its end', () => {
    // The durable row the daemon writes is what takes the badge back down —
    // without this the reading would latch for the life of the transcript.
    const items = [
      bash('c1', 'pnpm dev', { run_in_background: true }),
      reply('c1', LAUNCHED('bash_1')),
      item('shell_info', { callId: 'c1', workId: 'bash_1', open: false }),
    ];

    expect(hasRunningBackgroundShell(items, OWNS_THEM)).toBe(false);
  });
});
