...
/**
 * Every shell the MAIN thread records, oldest first, folded to how it stands
 * now.
 *
 * **A delegate's shells are its own, and this REVERSES the rule that shipped
 * first.** The old reading was that a delegate's `pnpm build` is still a
 * process on the user's machine, so it belonged in a list answering "what is
 * running" — which is true of one delegate and false of a fan-out. REPORTED as
 * "we should not show terminals from subagents", over a review that had ten
 * reviewers out at once: in a 1:1 chat every row here carries `nodeId: null`,
 * so a delegate's commands land in the SAME bucket as the agent's own and the
 * `Terminals` chip counts them together, with nothing on the row saying which
 * thread launched it. What the reader wanted from the chip — the commands the
 * agent they are talking to has in flight — was then the minority of a list
 * they could not filter by eye.
 *
 * Withholding them costs nothing that is not answered better elsewhere: a
 * delegate's own block holds its tool rows in full, its card in the agents
 * panel is where its state is read, and the run row's `shellsOpen` still counts
 * every detached command whoever launched it — so a fan-out holding the turn
 * open is still SAID, on the surface that says it.
 *
 * The drop is at the ITEM, before anything is indexed, so a delegate's launch
