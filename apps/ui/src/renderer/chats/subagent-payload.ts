 ...
 *
 * TWIN PARSER, and it has TWO writers on the daemon side rather than one.
 * `apps/daemon/src/v1/agents/utils/event-to-item.ts` stamps it from the
 * `parent_tool_use_id` its claude mapper reads off the stream-json line — that
 * is every ordinary row. `ChatService.attributeCard` stamps it onto the five
 * render-family CARDS, which are written by the host-tool sinks and never pass
 * through that mapper; there the value is INFERRED from live turn state,
 * because the MCP endpoint is addressed per node and names no thread. An item
 * payload is `z.unknown()` on the wire BY DESIGN — every item kind carries a
 * different shape — so no generated type carries it here and this is an
 * independent reading of the same shape. Renaming the key on either side means
