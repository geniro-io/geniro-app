...
/**
 * The reasoning row's text: how much thinking this TURN has cost, and for how
 * long.
 *
 * Both numbers are turn-scoped. The token count used to restart every time the
 * agent broke off to write, which read as the count going backwards; and there
 * was no duration at all, so a long silent stretch gave no sense of progress.
 * Elapsed is computed at render rather than published, so it advances with the
 * deltas that keep arriving during the stretch.
 */
function thinkingLabel(state: LiveState): string {
  const tokens = `${formatTokens(state.thinkingTokens ?? 0)} tokens`;
  if (state.thinkingSince === null) {
    return `Thinking… ${tokens}`;
  }
  const seconds = Math.max(
    0,
    Math.round((Date.now() - state.thinkingSince) / 1000),
  );
  const elapsed =
    seconds < 60
      ? `${seconds}s`
      : `${Math.floor(seconds / 60)}m ${seconds % 60}s`;
  return `Thinking… ${tokens} · ${elapsed}`;
}
...
export function withLiveText(
  ...
  for (const [key, state] of liveText) {
    ...
    if (state.text === '' && state.thinkingTokens === null) {
      continue;
    }
    const nodeId = key === CHAT_LIVE_KEY ? null : key;
    ...
    // the token total rides the `reasoning` kind — already styled as the
    // muted aside this is.
    const thinking = state.text === '' && state.thinkingTokens !== null;
    const entry: ItemEntry = {
      ...
      item: {
        ...
        kind: thinking ? 'reasoning' : 'message',
        role: 'assistant',
        payload: {
          text: thinking ? thinkingLabel(state) : state.text,
        },
        createdAt: '',
      },
    };
