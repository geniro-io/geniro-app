 ...
 * (`ask_user_question`, `report_findings`) ride it for the same reason: each
 * draws its own card, and the raw call beside it is the same row twice.
 *
...
function ownerOf(entry: TranscriptEntry): EntryOwner | typeof NO_OWNER {
  ...
  if (entry.type === 'task-list' || entry.type === 'findings') {
    return { nodeId: entry.nodeId, subagentId: entry.parentToolUseId };
  }
...
export function countTools(entries: readonly TranscriptEntry[]): number {
  return entries.reduce((sum, entry) => {
    ...
    if (
      ...
      // Neither card holds tool invocations: the calls that produced them are
      // hidden BY them, and counting those would report work with no row to
      // open.
      return sum;
    }
