...
import { CallQuestionCard } from './call-question-card';
...
import type { ErrorRecovery } from './error-payload';
import {
  errorFactsOf,
  errorRecoveryOf,
  errorReportText,
...
/**
 * Which recovery an error row offers, given the daemon's verdict and what this
 * chat can actually do.
 *
 * ONE of them, ranked: the daemon's recognised cure beats the general one. A
 * lapsed account session is not fixed by reopening the conversation — the
 * reopened session would fail on its next turn for the same reason — so
 * offering retry there sends the user round a loop, and offering both puts the
 * weaker answer beside the right one. Sign-in is never guessed at from the
 * message text either: the verdict is the daemon's.
 *
 * A pure function beside the component rather than inside it: the ranking is
 * the whole of the decision, and it is worth being able to state it in a test
 * without a rendered row.
 */
export function errorRecovery(
  verdict: ErrorRecovery | null,
  signIn: (() => void) | null,
  retry: (() => void) | null,
):
  | { label: string; icon: React.ReactNode; title: string; onClick: () => void }
  | undefined {
  if (verdict === 'cli-login') {
    // Sign-in or NOTHING — never a fall-through to retry, per the ranking above.
    if (!signIn) {
      return undefined;
    }
    return {
      label: 'Sign in',
      icon: <LogIn aria-hidden="true" className="size-3.5 shrink-0" />,
      // Says what now HAPPENS: the sign-in runs here and reports above the
      // composer, so a title promising a terminal would describe the mechanism
      // this row was fixed to stop using.
...
export const TranscriptItem = memo(function TranscriptItem({
  ...
  switch (item.kind) {
    ...
    case 'tool_call': {
      ...
      // The files it named, when it disclosed nothing else — an ACP agent
      // routinely discloses no arguments, so without this its rows say only
      // that a tool ran.
      const body =
        toolInputBody(
          name,
      ...
      return (
        <MessageBubble variant="note">
          ...
          <span>
            ...
            {/* The shared exact format, NOT a `toFixed(4)` of its own: this
                read `$0.4200` while the header two hundred pixels up said
                `$0.42` — one number, two notations, one screen. */}
            {typeof cost === 'number' ? ` · ${formatExactUsd(cost)}` : ''}
          </span>
        </MessageBubble>
      );
    }
    ...
    case 'call_question': {
      ...
      // user) is expected to answer it via answer_agent. Drawn as the question
      // card it is, inert: see {@link CallQuestionCard} for why it borrows the
      // live card's shape and not its promise.
      const raw = (item.payload as { options?: unknown } | null)?.options;
      const options = Array.isArray(raw)
        ? raw.filter((option): option is string => typeof option === 'string')
        : [];
      return (
        <CallQuestionCard
          question={payloadString(item.payload, 'question') ?? ''}
          options={options}
          caller={nodeName(payloadString(item.payload, 'callerNodeId'))}
          callee={nodeName(payloadString(item.payload, 'calleeNodeId'))}
        />
      );
    }
    case 'call_answer': {
