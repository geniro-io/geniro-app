...
describe('scanTurns + openTurnWorkedMs', () => {
  ...
  it("keys a chat's open turn to the chat sentinel", () => {
    const items = [userAt('2026-08-14T10:00:00.000Z')];

    const { open } = scanTurns(items);

    expect(open).toHaveLength(1);
    expect(open[0]?.agentKey).toBe(CHAT_AGENT_KEY);
  });

  it("keys each workflow node's open turn to that node, so a fan-out is separable", () => {
    // The defect: the scan held this key and dropped it, so two agents working
    // at once produced two OpenTurns nothing could tell apart — and a per-agent
    // duration cannot be attributed from an unlabelled list.
    const items = [
      userAt('2026-08-14T10:00:00.000Z'),
      item('status', '2026-08-14T10:00:01.000Z', {
        nodeId: 'reviewer',
        payload: { nodeId: 'reviewer', status: 'running' },
      }),
      item('status', '2026-08-14T10:00:02.000Z', {
        nodeId: 'builder',
        payload: { nodeId: 'builder', status: 'running' },
      }),
    ];

    const { open } = scanTurns(items, { nodeScoped: true });

    expect(open.map((turn) => turn.agentKey).sort()).toEqual([
      'builder',
      'reviewer',
    ]);
    // The chat-scoped row is filed under the other mode and must not appear.
