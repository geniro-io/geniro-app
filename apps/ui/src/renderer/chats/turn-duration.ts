...
export interface OpenTurn {
  /**
   * Which agent is taking this turn — a workflow node's id, or
   * {@link CHAT_AGENT_KEY} for a 1:1 chat, whose rows carry no `nodeId`.
   *
   * A SETTLED turn is attributed through a join rather than a stored field: its
   * figure is keyed by the terminal item's id, and that item's `nodeId` names
   * the agent. An open turn has no terminal item, so it had no identifier at
   * all — the scan held the key in hand and dropped it, and a fan-out's live
   * turns were therefore indistinguishable from one another.
   */
  agentKey: string;
...
export function scanTurns(
  ...
  const open: OpenTurn[] = [];
  // `.entries()` rather than `.values()`: the thread key IS the agent, and a
  // chat's null key is the sentinel rather than an absence — the same mapping
  // `Chats.tsx` already spells where it folds these by agent.
  for (const [nodeId, state] of threads.entries()) {
    if (state.startedAt !== null) {
      open.push({
        agentKey: nodeId ?? CHAT_AGENT_KEY,
