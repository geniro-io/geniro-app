...
): { latest: () => string[]; setKind: (kind: CliKind | null) => void } {
  let latest: string[] = [];
...
describe('useAgentVocabulary', () => {
  it('drops the previous kind’s list while the new kind is still being fetched', async () => {
    ...
    expect(probe.latest()).toEqual(['opus', 'sonnet', 'haiku']);
    ...
    expect(probe.latest()).toEqual(['cursor-grok-4.5', 'composer-2.5']);
  });
  ...
  it('serves a cached kind from the cache on return, with no second fetch', async () => {
    ...
    expect(probe.latest()).toEqual(['composer-2.5']);
    ...
    expect(probe.latest()).toEqual(['opus', 'sonnet']);
  ...
  it('clears the list when the kind goes away entirely', async () => {
    ...
    expect(probe.latest()).toEqual(['opus']);
