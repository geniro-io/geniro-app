...
export interface ChatRunState {
  ...
  /**
   * While {@link awayFromTail}, load the page AFTER the newest item on screen —
   * the forward twin of {@link loadOlder}. Resolves true when rows were
   * appended; reaching the conversation's end clears `awayFromTail`, so live
   * items resume.
   */
...
export function useChatRun(scope: ChatRunScope): ChatRunState {
  ...
  /** {@link loadingOlderRef}'s forward twin, for {@link loadNewer}. */
  ...
  const loadAround = useCallback(
    async (seq: number): Promise<boolean> => {
      ...
      try {
        ...
        // The window is FILLED FORWARD when the target sits near the start: the
        // backward read above only holds what lies below `seq + context`, so a
        // jump to a conversation's FIRST message used to load ~50 rows and
        // stop — REPORTED as "он просто подгружает какие-то первые сообщения, а
        // не всю историю", with the rest of the thread unreachable except by
        // leaving the window altogether. The room left in the page is taken
        // from what follows, oldest first.
        //
  ...
  /**
   * Read ON from a window in the middle of the conversation: append the page
   * after the newest item held.
   *
   * {@link loadAround} leaves the reader at a point, and the only way on from
   * there used to be "Latest", which drops the window for the tail — so the
   * stretch between a jumped-to message and the end could not be read at all
   * past the first page. This is {@link loadOlder} pointed the other way, with
   * the same rules: the guard is a ref, the append keeps only rows still newer
   * than the newest held (a scroll event between the fetch and React's commit
   * asks for the same page twice), and nothing is ever trimmed.
   *
   * Reaching the END is what ends the away state: one row past a page answers
  ...
  const loadNewer = useCallback(async (): Promise<boolean> => {
    ...
    const newest = itemsRef.current[itemsRef.current.length - 1];
    if (newest === undefined) {
      return false;
    }
    ...
    try {
      ...
      // As `loadOlder`: a page that will not load is a transcript that stops
      // growing downward, never a banner. The next scroll asks again.
      return false;
    } finally {
      loadingNewerRef.current = false;
    }
