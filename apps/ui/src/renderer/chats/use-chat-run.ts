...
import { conversationReplaced } from './compaction-payload';
...
export function useChatRun(scope: ChatRunScope): ChatRunState {
  ...
  const addItem = useCallback((item: ChatItem, live: boolean): void => {
    ...
    //
    // Geniro's own compaction alone. A CLI that compacts for itself REPORTS
    // what it left behind, and that reading arrives around its own marker row
    // rather than safely after it — clearing on one of those would wipe the
    // very figure the ring is waiting for.
    //
