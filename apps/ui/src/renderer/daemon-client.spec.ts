...
describe('parseChangedRuns — the runs_changed twin', () => {
  it('reads every row the daemon re-filed', () => {
    const row = { id: 'r1', createdAt: 'now', archivedAt: 'then' };
    expect(parseChangedRuns({ runs: [row] })).toEqual([row]);
  });

  it('drops a malformed row alone rather than the whole batch', () => {
    const row = { id: 'r1', createdAt: 'now' };
    expect(
      parseChangedRuns({
        runs: [row, null, { id: '' }, { id: 'r2' }, 'r3'],
      }),
    ).toEqual([row]);
  });

  it('answers empty for anything that is not the envelope', () => {
    expect(parseChangedRuns(null)).toEqual([]);
    expect(parseChangedRuns({})).toEqual([]);
    expect(parseChangedRuns({ runs: 'r1' })).toEqual([]);
  });
});
