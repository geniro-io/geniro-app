 ...
 *  edge kind (each annotation kind renders through its own component;
 *  anything else is data flow, matching graph-doc's fromFlow). */
 ...
 *  or instruction edge never eats into a data rule's arity (mirrors the
 *  daemon's per-kind count keys). Unknown peers are dangling edges React Flow
 *  won't render — skipped rather than flagged. */
function countByRule(
  edges: readonly { peerId: string; edgeKind: EdgeKind }[],
 ...
 * alone — the two annotation kinds carry no prompt context, so saying an
 * instruction edge "cannot feed" a node names the one thing it never does.
 * Twin of `edgeVerb` in the daemon's `graph-validate.ts`.
 */
...
function sideErrors(
  ...
  for (const [key, count] of counts) {
    const [edgeKind, peerKind] = key.split(' ') as [EdgeKind, string];
    const rule = rules.find((r) => r.edge === edgeKind && r.kind === peerKind);
    const verb = edgeVerb(edgeKind);
    if (!rule) {
      errors.push({
        type: 'connection',
        side,
...
export function validateNode(
  ...
  //
  // An incoming INSTRUCTION edge does NOT count, for the reason the daemon
  // gives: a block hands text to its target rather than putting it on a
  // trigger path, so an agent wired only to one is still an agent nothing
  // will ever start. Asking for "any incoming edge" here would pass a graph
  // the run then refuses, which is the one thing this mirror must not do.
  if (node.kind === 'agent' && !hasIncomingData && !hasIncomingCall) {
    errors.push({
      type: 'connection',
      side: 'input',
      message:
  ...
  // A block wired to nobody is inert — the daemon accepts it (it strands
  // nothing) so there is no run-gate mirror here, but it is silently doing
  // nothing, which is worth one line beside the field.
  if (node.kind === 'instruction' && outgoing.size === 0) {
    errors.push({
      type: 'connection',
      side: 'output',
      message:
  ...
  // `instructions` is the first node field whose CONTENT the daemon can
  // refuse, and the builder autosaves the whole workflow in one PUT — so an
  // over-long or control-character-carrying block does not fail its own field,
  // it stops every later edit to that workflow from persisting at all, and
  // leaving the page then discards the session's work. Mirrored here for the
  // same reason the connection rules are: a card that turns red before the
  // write is attempted beats a save that silently stops working.
  if (node.kind === 'instruction') {
    if (node.instructions.length > MAX_CUSTOM_INSTRUCTIONS_CHARS) {
      errors.push({
        type: 'config',
        message: `Over ${MAX_CUSTOM_INSTRUCTIONS_CHARS.toLocaleString()} characters — this block will not save.`,
      });
    }
    if (hasControlCharacters(node.instructions)) {
      errors.push({
        type: 'config',
        message:
