...
export function useNodeMcp(
  ...
  // The daemon answers a cold read at once with empty rows under this flag
  // and finishes the dial behind it. Without asking again the inspector would
  // keep that placeholder forever and render it as "no servers".
  const stillReading = listing?.pending === true;
  // The SHARED loop, not a second copy — the two MCP hooks each carrying
  // their own is how the same fire-once defect came to exist twice.
  const rereadToken = usePendingRetry(answered, stillReading);
