...
import { aProject } from '../tasks/__tests__/fixtures';
...
const projectsApi = vi.hoisted(() => ({
  // Annotated rather than inferred: an unannotated `Promise.resolve([])`
  // infers `never[]`, and every later `mockResolvedValue` of a real project
...
describe('Settings — custom instructions', () => {
  ...
  it('does not flush an unsavable value that followed a legal one', async () => {
    // A LEGAL edit arms the debounce, an unsavable edit then clears it — and if
    // the timer ref is merely cleared rather than nulled, the unmount flush
    // still sees a handle and writes the unsavable text. Nothing must be
    // written at all here, because the legal value was superseded and the
    // current one is unsavable.
    await mount();
  ...
  it('saves text of any length — far past the old 16,000-character cap', async () => {
    // The control: the guard must refuse only what the schema refuses, and
    // instruction text carries no length limit anywhere.
    const long = 'x'.repeat(20_000);
    await mount();
    await typeInstructions(long);

    // No refusal on screen either.
...
describe('the Projects pane', () => {
  it('lists the projects the daemon holds', async () => {
    projectsApi.listProjects.mockResolvedValue([
      aProject({ name: 'Geniro', folder: '/Users/me/code/geniro' }),
    ]);

    await mount('projects');

    expect(projectsApi.listProjects).toHaveBeenCalled();
    expect(container?.textContent).toContain('Geniro');
  });

  it('deletes through the daemon once the confirmation is accepted', async () => {
    // The pane's own spec covers the confirm in isolation; this one is the
    // wiring — that accepting it actually reaches the daemon and drops the row,
    // which no test drove before.
    projectsApi.listProjects.mockResolvedValue([aProject({ name: 'Geniro' })]);
    await mount('projects');

    const trash = [...(container?.querySelectorAll('button') ?? [])].find(
      (node) => node.getAttribute('aria-label') === 'Delete Geniro',
    ...
    expect(projectsApi.deleteProject).toHaveBeenCalledWith({
      projectId: 'p1',
    });
