...
import { newRunConfigId, replaceRunConfig } from '../chats/run-config';
import { RunConfigPicker } from '../chats/run-config-picker';
...
export function Settings({
  ...
  /**
   * The projects, read here rather than handed down from the board: this pane
  ...
  /**
   * The user's named agent configurations, read here on the same terms as the
   * fast actions above — this screen owns its own data and works whether or not
   * a chat has ever been opened.
   */
  const [configProfiles, setConfigProfiles] = useState<ConfigProfile[]>([]);
  /**
   * The saved run configurations, read here on the same terms as the actions
   * above — this screen owns its own data. They are ALSO reachable from the
   * chat sidebar's `+`, which is the surface for USING one; this is the surface
   * for keeping them in order, and both write the one `runConfigs` key.
   */
  const [runConfigs, setRunConfigs] = useState<RunConfig[]>([]);
  /**
   * What the run-configuration editor's own chips need: the workflow library
   * for the target picker, and the folder recents for the two directory
   * pickers. Read here rather than passed down, on the same terms as the
   * configurations themselves — this screen is reachable without a chat ever
   * having been opened.
   */
  ...
  useEffect(() => {
    ...
    void apis.projects
      ...
      .catch(() => {
        // A pane that cannot list projects shows its empty state. There is
        // nothing the user can do here about a daemon that is not answering,
        // and the connection banner above already says so.
      });
    return () => {
      cancelled = true;
    };
  ...
  const createProject = useCallback(
    (input: CreateProjectDto) => {
      ...
      void apis.projects
        .createProject({ createProjectDto: input })
        .then((project) => {
          setProjects((current) => [...current, project]);
        })
        .catch((err: unknown) => {
          // A folder already bound to a project, or one that does not exist,
          // is refused by the daemon in words the user can act on.
          setProjectsError(
            daemonErrorDetail(err) ??
              (err instanceof Error ? err.message : String(err)),
          );
  ...
  /**
   * Write the whole configuration list, rolling back on refusal — the same
   * shape as {@link persistFastActions} and for the same reason: `persist`
   * swallows the rejection, so a rollback chained onto it could never fire and
   * the screen would keep showing a profile settings.json does not have.
   *
   * Not folded into one helper with its twin. They write DIFFERENT keys, and
   * the generic version would take the key plus a setter plus the previous
   * value — three arguments to save four lines, at the cost of a call site
   * where the key being written is no longer visible.
   */
  const persistConfigProfiles = useCallback(
    (next: ConfigProfile[], previous: ConfigProfile[]): void => {
      setConfigProfiles(next);
      setError(null);
      void window.geniro
        .updateSettings({ configProfiles: next })
        .then(flashSaved)
        .catch((err: unknown) => {
          setConfigProfiles(previous);
          setError(String(err));
        });
    },
    [flashSaved],
  ...
  /**
   * The saved run configurations, on exactly the terms the fast actions above
   * are written — direct `updateSettings` with a rollback, for the reason
   * spelled out at {@link persistFastActions}.
   *
   * They are a SECOND list and a second pane, never a merge: a configuration
   * says where and with what a chat runs, an action says what it says. Folding
   * the two into one feature is what deleted the configurations once, taking
   * the user's saved setups with them.
   */
  ...
  /**
   * The two capability lookups the run-configuration editor asks per CLI, off
   * the ONE capabilities read above rather than a second fetch.
   *
   * `null` from either means "not known yet", which the chips draw as a wait
   * rather than as a refusal — the distinction the composer already relies on.
   */
  ...
  const approvalModesFor = useCallback(
    (kind: CliKind): readonly ChatApprovalMode[] | null =>
      ...
      <div
        ...
        <div className="mx-auto flex w-full max-w-2xl flex-col gap-6 px-6 py-8 max-sm:gap-5 max-sm:px-4 max-sm:py-5">
          ...
          {section === 'projects' ? (
            ...
            <RunConfigPicker
              // The chat sidebar's dialog rendered inline — same list, same
              // editor, same `runConfigs` key. No `onApply`: this pane keeps
              // them, the `+` starts from them.
              asDialog={false}
              open
              configs={runConfigs}
              ...
              // screen — the one honest answer here.
              captureCurrent={() => null}
          ...
          button press with no visible outcome. It falls through to here. */}
                  ...
                  agentSettings={{
                    claude: (
                      <>
                        ...
                        />
                        {/* Asked for HERE rather than as a section of its own
                            ("should be in claude settings menu"), which is also
                            the rule the switch above already follows. It is on
                            the claude card ALONE because the config directory
                            only changes the account for claude — the daemon
                            says so itself, and `configDirReasonFor` renders
                            cursor's picker inert on the same fact. */}
                        <ConfigProfileList
                          profiles={configProfiles}
                          onChange={(next) =>
                            persistConfigProfiles(next, configProfiles)
                          }
                          onPickDirectory={() =>
              ...
              It is its own ROW now rather than three controls crowded into one
              cell beside the switch: it does not SET the setting above, it
              checks whether the OS is honouring it, and the result line has
              somewhere to land that is not under an unrelated description. */}
                  <SettingsPanelRow
                    label="Delivery"
                    description={
              ...
              <section className="flex flex-col gap-3">
                ...
                <SettingsPanel>
                  <SettingsPanelRow
                    ...
                    description="A turn's tool calls start folded, file edits included. Every row is still there — one press opens it, and the group header still counts them.">
                    <Switch
                    ...
                    <SettingsPanelRow layout="block">
                      <details data-slot="host-preamble">
                        ...
                        <p className="mt-2 text-xs text-muted-foreground">
                          The CLIs are built for a terminal and say so in their
                          own system prompt, so geniro corrects that before your
                          instructions. You cannot edit it — it describes how
                          this app actually renders a reply.
                        </p>
              ...
              <section className="flex flex-col gap-3">
                ...
                <SettingsPanel>
                  <SettingsPanelRow
                    ...
                    description="Checks GitHub Releases on launch and every few minutes. Nothing is installed until you press Update now, and nothing restarts until you press Restart now.">
                    <Switch
                  ...
                  {/* The status line is this row's DESCRIPTION rather than a paragraph
              under the section: it is what the buttons beside it are about, and
              as a loose line it read as a third note among the two above it. */}
                  <SettingsPanelRow
                    label="Status"
                    description={
              ...
              <section className="flex flex-col gap-3">
                ...
                <SettingsPanel>
                  <SettingsPanelRow
                    label="Attach a debugger to the daemon"
                    ...
                    description={
                      <>
                        ...
                        daemon, and while the port is open any process on this
                        machine can run code inside it.
                      </>
                    }>
                    {storedInspect === null ? (
              ...
              button again, one nav click further away. */}
                  <SettingsPanelRow layout="block">
                    <p className="text-xs text-muted-foreground">
                      ⌥⌘L opens this app’s own debug log — the daemon, agent and
                      UI lines you can copy into a bug report.
                    </p>
                  </SettingsPanelRow>
                </SettingsPanel>
              </section>
