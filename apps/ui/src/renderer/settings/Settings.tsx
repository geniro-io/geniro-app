...
/**
 * The theme rows, built from the manifest rather than written out — a theme
 * added there appears here with no edit, which is what makes the list of theme
 * files the single source.
 */
const THEME_OPTIONS: SelectGroup[] = [
  {
...
export function Settings({
  ...
  const onThemeChange = useCallback(
    (next: string): void => {
      ...
      setTheme(next);
      // Applied here as well as persisted, and the two are not the same write:
      // main re-applies `nativeTheme.themeSource` when the patch lands, which
      // moves this renderer's `prefers-color-scheme` and would repaint on its
      // own — but only for a theme the media query can name. This is what makes
      // the change immediate for any theme, including one that is neither.
      setThemePreference(next);
      // Rolled back on refusal, unlike the switches on this screen, and for a
      // reason none of them have: the page is repainted HERE while the OS
      // chrome is main's to repaint when the write lands. A swallowed rejection
      // would leave the two disagreeing — a dark page under light traffic
      // lights — which is the split main-side application exists to prevent.
      setError(null);
      void window.geniro
        ...
        <div className="mx-auto flex w-full max-w-2xl flex-col gap-6 px-6 py-8">
          ...
          button press with no visible outcome. It falls through to here. */}
              ...
              <section className="flex flex-col gap-3">
                ...
                <div className="flex items-center gap-3">
                  <Label htmlFor="settings-theme" className="shrink-0">
                    Theme
                  </Label>
                  <Select
                    id="settings-theme"
                    className="w-52"
                    groups={THEME_OPTIONS}
                    value={theme}
