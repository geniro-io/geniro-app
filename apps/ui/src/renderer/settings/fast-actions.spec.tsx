...
function render(
  ...
  rerender: (
    next: Partial<React.ComponentProps<typeof FastActionsPane>>,
  ) => void;
} {
  const onSave = vi.fn();
  const onDelete = vi.fn();
  const paint = (
    extra: Partial<React.ComponentProps<typeof FastActionsPane>>,
  ): void => {
    act(() => {
      root.render(
        <FastActionsPane
  ...
  paint({});
  return { onSave, onDelete, rerender: paint };
}
...
describe('FastActionsPane — list', () => {
  it('pressing a row opens THAT action in the editor', () => {
    // The row's press is an edit, not a launch: this screen is where actions
    // are set up, and the buttons that start a chat from one live under the
    // composer (`chats/fast-action-bar.tsx`).
    render({ configs: [config({ name: 'Geniro app' })] });

    click(button('Edit the fast action “Geniro app”'));

    const input = container.querySelector<HTMLInputElement>(
      'input[aria-label="Fast action name"]',
    );
    expect(input?.value).toBe('Geniro app');
  });
...
describe('FastActionsPane — editor', () => {
  ...
  it('a new configuration saves against a null id', () => {
    ...
    const { onSave } = render({ seed: captured });
    ...
    expect(onSave).toHaveBeenCalledWith(
      expect.objectContaining({
        name: 'Fresh',
        // Seeded from the choices the next chat would open with, so "new" is a
        // naming step rather than six pickers — this fails if the seed is
...
describe('FastActionsPane — editor guards', () => {
