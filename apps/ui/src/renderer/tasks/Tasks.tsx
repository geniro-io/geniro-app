...
import type { CliDetection, DaemonHandle } from '../../shared/contracts';
...
export function Tasks({
  ...
  /**
   * The workflow library and which CLIs this machine has, for the two target
   * pickers (the project chip, and a card's own Agent row).
   *
   * Re-read on the `active` transition rather than once at mount, the shape
   * `Chats` uses for the same list and for the same reason: this screen is
   * latch-mounted, the library is editable on the Workflows page while it
   * stays mounted, and a CLI can be installed while the app is open. A failed
   * read is the EMPTY list and never an error on this screen — a board is
   * perfectly usable while the pickers offer less than they could.
   */
    ...
    <div className="flex h-full min-h-0">
      ...
      {openTask !== null ? (
        <TaskDetail
          ...
          onFollowUp={(text) => {
            void board.runTask(openTask.id, text);
          }}
