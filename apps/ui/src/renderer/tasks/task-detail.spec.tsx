...
describe('priority and due date', () => {
  ...
  it('renders the report the agent sent through the transcript’s own markdown pipeline', () => {
    // The report is the card's own field now — written by the agent with
    // `update_task` — rather than a transcript row fetched from the run.
    const el = detail({
      report: 'Fixed the *cap*.',
  ...
  /** The report section's caption, found by its alarm tone. */
  const alarmCaption = (el: HTMLElement): Element | null =>
    el.querySelector('h3.text-destructive');

  it('frames the report of a FAILED card as a failure', () => {
    // REPORTED as "for report with error it should have design with some
    // problem". The agent moved its own card to `failed` and said why.
    const el = detail({
      status: 'failed',
      report: 'Could not reproduce the crash on this branch.',
    });

    expect(alarmCaption(el)?.textContent).toBe('Failed');
    // Case-SENSITIVE and on the source word: the caption is uppercased in CSS,
    // so `textContent` still reads `Report` if the framing regresses.
    expect(el.textContent).not.toContain('Report');
    expect(el.textContent).toContain('Could not reproduce the crash');
  });

  it('says so even when the failed run left no report behind', () => {
    // The commonest failure: the agent died before it could call the tool.
    // With the section withheld the status chip was the only word on it.
    const el = detail({ status: 'failed', report: null });

    expect(alarmCaption(el)?.textContent).toBe('Failed');
    expect(el.textContent).toContain('left no report');
  });

  it('draws NO failure section for a card that merely has no report', () => {
    const el = detail({ status: 'in_review', report: null });

    expect(alarmCaption(el)).toBeNull();
    expect(el.textContent).not.toContain('left no report');
  });
});
